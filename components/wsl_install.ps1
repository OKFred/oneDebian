...
# Description: 下载并导入 Debian rootfs 为 WSL 2 发行版 (支持自定义名称 & 本地镜像缓存复用)
# Author: Fred
# ==============================================================================
...
try {
    ...
    if (Test-Path -LiteralPath $archivePath) {
        $cacheSizeMB = [math]::Round((Get-Item -LiteralPath $archivePath).Length / 1MB, 2)
        if ($cacheSizeMB -gt 1) {
            $useCache = $true
        }
    }

    $rootfsUrl = "N/A (Local Cache)"
    $imageUrl = $null
    ...
    if ($useCache) {
        Write-Host "    $(if ($global:CurrentLang -eq 'zh-CN') { '镜像来源' } else { 'Image Source' }): [⚡ 本地缓存镜像复用] ($archivePath)" -ForegroundColor Green
    } else {
    ...
    # 4. 下载或复用镜像
    if ($useCache) {
        Write-Host "`n[1/3] $(if ($global:CurrentLang -eq 'zh-CN') { "发现本地已有镜像缓存，直接复用免下载: $archivePath" } else { "Reusing local image cache: $archivePath" })" -ForegroundColor Green
    } else {
        Write-Host "`n[1/3] $(if ($global:CurrentLang -eq 'zh-CN') { "正在下载 Debian $Version rootfs 到本地缓存..." } else { "Downloading Debian $Version rootfs to cache..." })" -ForegroundColor Green
        ...
        if ($LASTEXITCODE -eq 0 -and $checksums) {
            ...
            if ($checksumLine) {
                ...
                if ($expected -ne $actual) {
                    Write-Host "`n[!] $(if ($global:CurrentLang -eq 'zh-CN') { '警告: SHA-256 哈希校验不匹配！重新准备缓存。' } else { 'Warning: SHA-256 checksum mismatch.' })" -ForegroundColor Yellow
                }
            }
        }
    }
